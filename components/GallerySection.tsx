...
export default function GallerySection({ invitation }: Props) {
  const hasCustomPhotos = Boolean(invitation?.galleryImages?.length);
  const photos: Photo[] = hasCustomPhotos
    ? buildPhotos(invitation!.galleryImages)
    : DEFAULT_PHOTOS;

  const subtitle = invitation?.gallerySectionSubtitle || "Anılar";
  const title = invitation?.gallerySectionTitle || "Fotoğraf Galerisi";
    ...
    <section id="gallery" className="section-gap relative overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          {!hasCustomPhotos && (
            <p className="font-sans text-xs mt-4" style={{ color: "rgba(107,95,88,0.5)" }}>
